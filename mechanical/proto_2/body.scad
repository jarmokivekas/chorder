...
module body(){
	difference(){
		...
		// extrude outer shape
		linear_extrude(height=120) import(file="body_top-down_profile_outer.dxf");
		// extrude inner shape
		linear_extrude(height=120) import(file="body_top-down_profile_inner.dxf");
		// diagonally cut away exces
...
module buttonhole(){
	rotate([0,0,45]) cube(size=[40,13,13], center=true);
